...
data1 = [0, 0, 0, 0]  # hannah, tracy kinetic ratio 
data2 = [0, 0, 0, 0]  # hannah, tracy effort ratio
labels = ['Human #1', 'Robot #1', 'Human #2', 'Robot #2']

# Create figure and subplots
plt.style.use('dark_background')
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(5, 10))
bar1 = ax1.bar(labels, data1, color=['red', 'blue', 'red', 'blue'])
bar2 = ax2.bar(labels, data2, color=['red', 'blue', 'red', 'blue'])
...
def update(frame):
    ...
    try:
        ...
        if abs(hannah_human_kinetic) < tol:
            ...
            data1[0] = hannah_human_kinetic 
                        
        if abs(hannah_robot_kinetic) < tol:
            data1[1] = 1
        else:
            data1[1] = 1* (hannah_robot_kinetic)

        if abs(tracy_human_kinetic) < tol:
            data1[2] = 1
        else:
            # data1[1] = tracy_robot_kinetic / tracy_human_kinetic
            data1[2] = tracy_human_kinetic

        if abs(hannah_robot_kinetic) < tol:
            data1[3] = 1
        else:
            data1[3] = 1 * (tracy_robot_kinetic)

        if abs(hannah_human_effort) < tol:
            data2[0] = 1
        else:
            # data2[0] = hannah_robot_effort / hannah_human_effort
            data2[0] = hannah_human_effort
            
        if abs(hannah_robot_effort) < tol:
            data2[1] = 1
        else:
            data2[1] = 1.0 * (hannah_robot_effort)

        if abs(tracy_human_effort) < tol:
            data2[2] = 1
        else:
            # data2[1] = tracy_robot_effort / tracy_human_effort
            data2[2] = tracy_human_effort
            
        if abs(tracy_robot_effort) < tol:
            data2[3] = 1
        else:
            data2[3] = 1.0 * (tracy_robot_effort)
